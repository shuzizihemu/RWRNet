...
def create_window(window_size, channel):
    ...
    window = _2D_window.expand(shape=(channel, 1, window_size, window_size))
    return window
...
def _logssim(img1, img2, window, window_size, channel, size_average=True):
    ...
    ssim_map = ((2 * mu1_mu2 + C1) * (2 * sigma12 + C2)) / ((mu1_sq + mu2_sq + C1) * (sigma1_sq + sigma2_sq + C2))
    ssim_map = (ssim_map - paddle.min(ssim_map)) / (paddle.max(ssim_map) - paddle.min(ssim_map))
    ssim_map = -paddle.log(ssim_map + 1e-8)
